  ...
  const [openMenu,setOpenMenu] = useState({id:0, open:false})
    ...
    return (
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
            ...
            <div>
                <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 q-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`} >
                    <Image className={expand ? 'w-5' : "w-6.5 mx-auto"} src={expand ? assets.pencil_icon : assets.phone_icon_dull} alt='' />
