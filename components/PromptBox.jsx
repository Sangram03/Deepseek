...
const PromptBox = ({ setIsLoading, isLoading }) => {
  ...
  const sendPrompt = async()=>{
    ...
    try {
      e.preventDefalt();
      if(!user) return toast.error ('Login to Sendf message');
    ...
    if(data.success){
      setchats((prevChats)=> prevChats.map((chat)=> chats._id === selectedChat._id ? {...chat ,messages:[...chat.messages,data.data]} : chat))
       ...
       className={`w-full ${false ? 'max-w-3xl' : "max-w-2xl"} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            <p className="flex items-center gap-2 text-xs border border-gray-300/40 px-1 rounded-full cursor-pointer hover:bg-gray-500/20 transition">
